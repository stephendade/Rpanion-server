...
class videoStream {
  ...
  getVideoDevices (callback) {
    ...
    if (this.deviceStream !== null) {
      ...
      if (this.videoSettings && this.devices) {
        ...
        // Access videoMediaDestination only if not null, otherwise return an empty string
        responseData.videoMediaDestination = this.videoSettings?.mediaDestination || '';
      }
    ...
    exec(`${pythonPath} ./python/get_camera_caps.py`, (error, stdout, stderr) => {
      ...
      try {
        ...
        return callback(null, {
          ...
          // Access stillMediaDestination only if not null, otherwise return an empty string
          stillMediaDestination: this.stillSettings?.mediaDestination || ''
        });
      } catch (e) {
  ...
  startPhotoMode(callback) {
    ...
    const defaultDestination = logpaths.mediaDir;

    if(!this.stillSettings.mediaDestination) {
      console.log('No media destination set, using default:', defaultDestination);
      this.stillSettings.mediaDestination = defaultDestination;
    }
  ...
  startVideoMode(callback) {
    ...
    if(!this.videoSettings.mediaDestination) {
      console.log('No media destination set, using default:', defaultDestination);
      this.videoSettings.mediaDestination = defaultDestination;
    }
